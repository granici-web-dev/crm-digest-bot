...
**Один сервис `app`.** aiogram, APScheduler и отправка в служебный бот живут в одном процессе. Compose v1: `app` + `postgres`, ежедневный `pg_dump` с хранением 30 дней. Делим `app` на `bot` и `scheduler`, только если появится вторая причина кроме «так было в ADR» (например, независимые рестарты или разная нагрузка).
