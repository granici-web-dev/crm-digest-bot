...
- Отступления от shape d2–d6: (1) `created_by_id` читается из `raw.created_by.id` в SQL `load_lead_frame`, а не в `prepare_lead_frame` (он получает строки без raw); тестовые строки снапшота кладут его в raw через `lead_snapshots_row`. (2) «Только Site» для d5 задан как `sources.site: ["Site"]` в status-mapping.yaml (подмножество web, валидатор), а не литералом в коде. (3) Тест `test_module_disabled_in_module_settings_is_skipped` теперь ждёт отправку без блока d1: раньше d1 был единственным модулем. (4) В d2 и d3 подпись «cel mai vechi / самый старый» стоит у первой группы, как в примере меню; у остальных в скобках их собственный максимум.
- Сейчас: прогон с DRY_RUN=1 в тестовой группе (неделя), уже с d1–d6.
...
- Daily реализован целиком (d1–d6; d7, d8 выключены). Weekly/monthly/yearly без реализованных модулей не отправляются (report_runs failed, алерт).
...
- Незнакомые ключи лида (unknown_raw_key) пишутся в raw, не вырезаются; алерт шлёт раннер отчёта. Остальные записи custom_field_mismatches (missing, unexpected_value, invalid_shape) в служебный бот пока не уходят.
...
- d2: касание = смена статуса, `last_contact_at ≠ created_at`, ofertat или лид создан консультантом с `active: true`; лид на `not_taken` или без консультанта «не взят» независимо от касаний. Условие «статус пуст» не используется: IN PROCES статус по умолчанию у лидов из API.
