...
- **m2, m4**: `lead_counts` и `lead_counts_by_showroom` на окне месяца, когорта на дату снапшота. Уровень SCR в m4: первая ступень из `params.levels` модуля m4 (имена порогов `kpi.yaml`) с SCR ≥ порога; ниже всех ступеней → «ниже минимума»; SCR = null → «—». Отметка цели по `targets.scr`, как в m5.
...
- **m8**: правило w4 на окне этого и прошлого месяца, оба по одному снапшоту. Лид, у которого после потери в прошлом месяце статус сменился ещё раз, из прошлого месяца выпадает: status_changed_at хранит только последнее изменение. Изменение = (этот − прошлый) / прошлый; прошлое значение 0 даёт «—».
